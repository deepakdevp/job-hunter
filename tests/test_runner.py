...
async def test_enrich_job_raw_fallback():
    html = '<html><body><p>Some random unstructured content that is long enough to pass the raw text extraction. ' + 'x' * 200 + '</p></body></html>'
    with patch("job_hunter.enrich.runner.fetch_page", new_callable=AsyncMock) as mock_fetch:
        mock_fetch.return_value = html
...
async def test_run_enrichment(tmp_path):
    ...
    db.upsert_job(Job(
        url="https://example.com/1", title="SWE", company="A",
        location="Tokyo", source="indeed", description=None,
    ))
    db.upsert_job(Job(
        url="https://example.com/2", title="FE", company="B",
        location="Osaka", source="indeed", description=None,
    ))
